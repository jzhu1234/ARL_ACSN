...
import math
import struct
from event_gen_CKN import event_gen
  ...
  def step(self):
    ...
    if self.state != -1:
      ...
      # Transmit message about current status
        ...
        message = str(self.timestamp)+' '+self.name+' '+self.type+' '+str(self.rand_rank)
        # Transmit message
        self.transmit(message)
        # Receive information
        data,length = self.receive()
    ...
    if len(self.ninfo) < k or cond_nbr:
      ...
      try:
        for node in self.ninfo:
          ...
          if float(self.ninfo[node][1]) >= self.rand_rank:
